...
namespace YouthMeadowGeneralStore
{
    ...
    {
        private async Task<bool> HandleSpecialDateEventsAsync(int month, int day)
        {
            if (month == 2 && day == 14)
            {
            ...
            if (month == 1 && day >= 25 && day <= 31 && _random.NextDouble() < 0.1)
            {
                ShowInfo(
                ...
                foreach (var item in goods.OrderBy(_ => _random.Next()).Take(_random.Next(1, goods.Length + 1)))
                {
            ...
            if (month == 1 && day >= 21 && day <= 31 && _random.NextDouble() < 0.1)
            {
                ...
                foreach (var item in flowers.OrderBy(_ => _random.Next()).Take(_random.Next(1, flowers.Length + 1)))
                {
                    _warehouse.Add(item.Clone());
                    _money -= 100m;
                    await EndBusinessDayAsync();
            ...
            if (month == 2 && day >= 1 && day <= 13 && !_matchmakingDone && _random.NextDouble() < 0.1)
            {
            ...
            if (month == 5 && day == 20 && _relationshipEstablished && !_event520Done)
            {
                _event520Done = true;
